<template>
  <div class="bovenbalk h-auto">
    <Loader :showLoader="isLoading"></Loader>
    <div class="row">
      <div
        class="col-12"
        v-if="!nieuwLid && gevuldeLedenLijst && !eigenProfiel"
      >
      ...
      <div class="col-6 col-sm-8 flex align-self-end flex-column">
        ...
        <div
          class="d-flex justify-content-start mt--05 text-align-left"
          v-if="!nieuwLid"
        >
          <label class="mt-2 lg:ml-4 text-sm"
            >Lidnr.: {{ lid.verbondsgegevens.lidnummer }}</label
          >
        </div>
      </div>

      <div
        v-if="gevuldeLedenLijst && !eigenProfiel"
      ...
      >
        <div class="flex justify-content-end">
          ...
          <Menu
            ...
            :popup="true"
            class="sub-menu-items p-4"
          >
            <template #item="{ item }">
              <div @click="gaNaar(item.link)">
                <i :class="item.icon" class="lid-menu-item mr-2"
                  ><label class="cursor-pointer lid-menu-item font ml-2">{{
                    item.label
                  }}</label></i
                >
              </div>
          ...
          <opslaan
            ...
            :disabled="!changes"
            @click="$emit('opslaan')"
            v-if="kanOpslaan || kanNieuwLidAanmaken"
          ></opslaan>
        </div>
      </div>
