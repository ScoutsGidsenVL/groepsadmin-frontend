    ...
    <card>
      <template #title>
        <div class="d-flex col-12 justify-content-between">
          <span> {{ title }}</span><span class="small mt-1" v-if="groepenLaden">Groepen laden &nbsp;<i class="fas fa-spinner fa-spin"></i></span>
        </div>
      </template>
      <template #content>
        <accordion :multiple="true">
          <accordionTab v-for="(geg, index) in gegVelden" :key="index">
            <template #header>
              <div class="d-flex col-12 justify-content-between">
                <span> {{ groepNaam(index) }} {{ groepNaam(index) ? ' - ' : '' }} {{ index }}</span>
              </div>
            </template>
            <dynamisch-veld :veld="gegVelden[index].schema" :model-value="gegVelden[index].waarden" :groepIndex="index" @changeValue="changeValue"></dynamisch-veld>
          </accordionTab>
        </accordion>
      </template>
    </card>
...
export default {
  ...
  methods: {
    groepNaam(index) {
      if (this.groepen && !this.$store.getters.groepenLaden && this.groepen[index]) {
