    ...
    <card>
      <template #title>
        <div class="d-flex col-12 justify-content-between">
          <span> {{ title }}</span>
          <Button icon="pi pi-plus" class="p-button-rounded add-button mt-1" @click="voegContactToe" title="Voeg adres toe" />
        </div>
          ...
          <accordionTab v-for="(contact, index) in contacten" :key="index">
            <template #header>
              <div class="d-flex col-12 justify-content-between">
                ...
                <Button icon="pi pi-trash" class="p-button-rounded p-button-outlined p-button-danger remove-button mr-1" @click="$event.stopPropagation(); remove(index)" title="Verwijder contact" />
            ...
            <base-input label="GSM" v-model="contacten[index].gsm" type="text"/>
            <base-dropdown
              :options="adresArray"
              label="Adres"
            ...
            <template v-for="(adres, index) in adressen" :key="index">
              <base-input label="Telefoon" v-model="adressen[index].telefoon" type="text" :disabled="true"  v-if="contacten[index] && adres.id === contacten[index].adres" />
            </template>
          </accordionTab>
        </accordion>
      </template>
  ...
  setup(props) {
    ...
    onUpdated(() => {
      ...
      state.adressen.forEach(adres => {
          state.adresArray.push({value: adres.id, label: adres.straat + " " + adres.nummer + ", " + adres.postcode + " " + adres.gemeente})
        }
      );
    })
