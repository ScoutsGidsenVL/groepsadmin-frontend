    ...
    <accordionTab v-for="(adres, index) in adressen" :key="index">
      <template #header>
        <div class="d-flex col-12 justify-content-between">
          <span @click="$event.stopPropagation(); showMarker(index)"><i class="pi pi-map-marker"></i> {{ setMarker(index)}} </span> <span>{{ setHeader(adres) }}</span>
          <i class="pi pi-envelope mr-3" v-if="adressen[index].postadres" title="postadres"></i>
          <Button v-if="!adressen[index].postadres" icon="pi pi-trash"
                  class="p-button-rounded p-button-outlined p-button-danger remove-button mr-1"
                  @click="$event.stopPropagation(); remove(index)" title="Verwijder adres"/>
        </div>
      </template>
      <base-input
      ...
      <straat-zoek-auto-complete
        ...
        v-if="adressen[index].land === 'BE'"
      />
      <BaseInput label="Telefoon" v-model="adressen[index].telefoon" type="text" :disabled="!bewerkbaar"/>
      <BaseCheckbox label="Postadres" multiple="false" v-model="adressen[index].postadres"
                    @change="zetPostadres(index)" :disabled="adressen[index].postadres || !bewerkbaar"/>
    </accordionTab>
  </accordion>
</template>
