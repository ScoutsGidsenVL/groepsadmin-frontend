<template>
  <div v-if="criteria && criteria.activated">
    ...
    >
      ...
      <div
        v-for="(geg, index) in gesorteerdeGroepseigenGegevens()"
        :key="index"
      >
        <div v-if="geg.items && geg.items.length > 0">
          <div
            class="select-kolom-header mb-1 full-width d-flex align-content-start"
          >
            {{ geg.label }}
          </div>
          <div v-for="(item, index) in geg.items" :key="index">
            <div class="d-flex align-content-start">
              <checkbox
                ...
                @change="activeerGroepseigenGegeven(item)"
              ></checkbox>
              <label class="text-align-left" :for="item.veld">{{
                item.label
              }}</label>
              <div
                v-if="item.vinkje"
                class="full-width d-flex justify-content-end"
              >
                <checkbox
                  :binary="true"
                  class="ml-4"
            ...
            <div v-if="!item.vinkje">
              ...
              <div
                class="d-flex justify-content-start margin-left-27px"
                v-if="item.keuze"
              >
                <dropdown
                  v-if="item.keuze"
...
export default {
  ...
  methods: {
    ...
    activeerGroepseigenGegeven(item) {
      if (!item.activated) {
        this.emitter.emit("deactiveerGroepseigenGegeven", {
    ...
    selecteerGroepseigenGegeven(veld, waarde, operator) {
      this.emitter.emit("changeGegKeuzeCriterium", {
        criteria: this.criteria,
        veld: veld,
        waarde: waarde,
        operator: operator,
