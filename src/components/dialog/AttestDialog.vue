    ...
    <template #header>
      ...
      >
        <span class="font-bold white-space-nowrap"
          >Fiscaal attest genereren</span
        >
      </div>
    </template>
    <div class="activiteit-content">
      <h4>Groep</h4>
      <div>Vul hier de toegangscode in die je van Belcotax ontving:</div>
      <br />
      <BaseInput
        v-model="attest.kbo"
    ...
    watch(
      ...
      (nieuweGroep) => {
        if (nieuweGroep?.instantie && state?.attest) {
          ...
          state.attest.certadres =
            nieuweGroep.instantie.adres.straat +
            " " +
            nieuweGroep.instantie.adres.nummer +
            " " +
            nieuweGroep.instantie.adres.bus;
