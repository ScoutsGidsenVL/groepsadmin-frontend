    ...
    <div class="container-fluid" ref='scrollComponent'>
      ...
      >
        <template #header>
          ...
          <label class="float-left mt--1"> {{ totaalAantalLeden }} rijen </label> <label v-if="aantalIds > 0"
                                                                                   class="float-left mt--1">&nbsp;(
          {{ this.aantalIds }} {{ this.aantalIds === 1 ? 'lid' : 'leden' }} geselecteerd )</label>
        </template>
        <template #empty>
          Geen leden gevonden op basis van de huidige filter.
        </template>
        <template #loading>
          Leden laden. Even geduld aub...
        </template>
        <column selectionMode="multiple" :exportable="false" headerStyle="width: 3em"
                @click.stop.prevent="selecteerLid"></column>
        <column v-for="kolom of actieveKolommen" :field="kolom.field" :header="kolom.header" :key="kolom.field">
          <template #header class="sticky-top position-sticky">
            <div class="custom-column">
              <a v-if="kolom.header === 'Lidgeld betaald aan SGV'" class="ml-1 float-right"
                 href="https://wiki.scoutsengidsenvlaanderen.be/doku.php?id=handleidingen:groepsadmin:paginas:groepsinstellingen&s[]=verzekerd"
                 target="_blank">
                <i class="fa fa-question-circle resolve info-button" title="meer info" style="margin-left: 3px"></i>
              </a>
            </div>
          </template>
          <template #body="slotProps">
            ...
            <div v-if="kolom.type === 'vinkje'" class="table-checkbox clickable">
              <i class="pi"
                 :class="{'true-icon pi-check-circle': isWaardeTrue(slotProps.data.waarden[kolom.field]), 'false-icon pi-times-circle': isWaardeFalse(slotProps.data.waarden[kolom.field])}"></i>
            </div>
          </template>
        </column>
        <template #footer v-if="isLoadingMore">
          <span class="small mt-1">Meer leden laden &nbsp;<i class="fas fa-spinner fa-spin"></i></span>
        </template>
      </data-table>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    getLeden() {
      this.offset === 0 ? this.isLoadingLeden = true : this.isLoadingMore = true;
    ...
    handleScroll() {
      if (this.$refs.scrollComponent && this.$refs.scrollComponent.getBoundingClientRect().bottom < window.innerHeight && !this.isLoadingMore) {
      ...
      this.huidigeFilter.kolommen.forEach(filterKolom => {
        this.kolommen.forEach(kolom => {
          if (kolom && kolom.id === filterKolom) {
            this.actieveKolommen.push({field: kolom.id, header: kolom.label, type: kolom.type})
          }
        })
      })
    },
