      ...
      <div class="lg:ml-8">
        <div class="lg:ml-8">
          <div class="row bovenbalk mt-7 mb-8">
            ...
            <div
              class="col-12 col-lg-6 col-xl-8 groep-select"
              v-if="!groepenLaden"
            >
              <BaseDropdown
          ...
          <div>
            <div class="col-9 pr-8 mt-10" v-if="ledenaantallenData">
              <div
                ...
                v-if="selectedGraph === 'Ledenaantallen'"
              >
                <div
                  class="d-flex justify-content-center"
                  v-if="ledenaantallenData.takStatistieken.length === 0"
                >
                ...
                >
                  ...
                  <tbody>
                    <tr
                      v-for="(
                        takStatistiek, key
                      ) in ledenaantallenData.takStatistieken"
                      :key="key"
                    >
                      ...
                      <td>
                        <Chart
                          type="bar"
                          :data="
                            eigenschappenData(takStatistiek.aantalLeden).data
                          "
                          :options="
                            eigenschappenData(takStatistiek.aantalLeden).options
                          "
                          class="historiek-chart"
                        />
                      </td>
                      <td>
                        {{ round(takStatistiek.percentageVrouwelijkeLeden, 0) }}
                      </td>
                      ...
                      <td>
                        <Chart
                          type="bar"
                          :data="
                            eigenschappenData(takStatistiek.aantalLeiding).data
                          "
                          :options="
                            eigenschappenData(takStatistiek.aantalLeiding)
                              .options
                          "
                          class="historiek-chart"
                        />
              ...
              <div id="Eigenschappen" v-if="selectedGraph === 'Eigenschappen'">
                <table class="table">
                  ...
                  <tbody>
                    <tr
                      v-for="(criterium, key) in ledenaantallenData.criteria"
                      :key="key"
                    >
                      <td class="text-align-left">{{ criterium.naam }}</td>
                      <td class="text-align-left">
                        {{ format(criterium.waarden["Nu"], criterium.formaat) }}
                      </td>
                      <td>
                        <Chart
                          type="bar"
                          :data="eigenschappenData(criterium.waarden).data"
                          :options="
                            eigenschappenData(criterium.waarden).options
                          "
                          class="historiek-chart"
                        />
                      </td>
              ...
              <div
                id="Groepsevolutie"
                v-if="selectedGraph === 'Groepsevolutie'"
              >
                <Chart
                  type="line"
                  :data="getGroepsevolutie.data"
                  :options="getGroepsevolutie.options"
                  class="w-75"
                />
              </div>
              <div
                id="Ledenaantal per leeftijd"
                v-if="selectedGraph === 'Ledenaantal per leeftijd'"
              >
                <Chart
                  type="bar"
                  :data="getLeeftijdsAantallen.data"
                  :options="getLeeftijdsAantallen.options"
                  class="w-75"
                />
              </div>
              <div
                id="Huidige leidingservaring"
                v-if="selectedGraph === 'Huidige leidingservaring'"
              >
                <Chart
                  type="doughnut"
                  :data="getLeidingservaring.data"
                  :options="getLeidingservaring.options"
                  class="w-30rem"
                />
              </div>
              <div
                id="Instroom en uitstroom per leeftijd"
                v-if="selectedGraph === 'Instroom en uitstroom per leeftijd'"
              >
                <Chart
                  type="bar"
                  :data="getInUitStroom.data"
                  :options="getInUitStroom.options"
                  class="w-75"
                />
              </div>
              <div
                id="Instroom"
                v-if="selectedGraph === 'Instroom per leeftijd'"
              >
                <Chart
                  type="bar"
                  :data="getInStroom.data"
                  :options="getInStroom.options"
                  class="w-75"
                />
              </div>
              <div
                id="Uitstroom"
                v-if="selectedGraph === 'Uitstroom per leeftijd'"
              >
                <Chart
                  type="bar"
                  :data="getUitStroom.data"
                  :options="getUitStroom.options"
                  class="w-75"
                />
              </div>
            </div>
...
export default {
    ...
    return {
      ...
      graphOptions: [
        ...
        { label: "Groepsevolutie", value: "Groepsevolutie" },
        {
          ...
          value: "Ledenaantal per leeftijd",
        },
        {
          label: "Huidige leidingservaring",
          value: "Huidige leidingservaring",
        },
        {
          label: "Instroom en uitstroom per leeftijd",
          value: "Instroom en uitstroom per leeftijd",
        },
  ...
  computed: {
    getLeeftijdsAantallen() {
      return LedenAantallenService.getLedenaantalPerLeeftijd(
        this.ledenaantallenData
      );
    },
    ...
    getLeidingservaring() {
      return LedenAantallenService.tekenHuidigeLeidingsErvaring(
        this.ledenaantallenData
      );
    },
