  ...
  steekkaartSpace() {
    ...
    onMounted(() => {
      ...
      RestService.getLid(state.id).then((response) => {
        ...
        state.lid.persoonsgegevens.geboortedatum = DateUtil.formatteerDatum(
          state.lid.persoonsgegevens.geboortedatum
        );
      });
    });
    ...
    const checkForm = () => {
      ...
      state.layout.forEach((veld) => {
        ...
        if (
          veld.label === "Telefoon/gsm:" &&
          state.steekkaartWaarden[veld.id]
        ) {
          let reformattedNumber = Telefoonnummer.formatNumber(
            state.steekkaartWaarden[veld.id]
          );
