...
export default {
  ledenlijstSpace() {
    ...
    const heeftSteekkaartleesRecht = () => {
      setTimeout(() => {
        return rechtenService.heeftSteekkaartLeesrecht(
          store.getters.profiel,
          "steekkaart"
        );
      }, 2000);
    };
    ...
    const deactiveerGroepseigenGegeven = (criteria, veld) => {
      if (state.huidigeFilter.criteria[criteria.criteriaKey]) {
        state.huidigeFilter.criteria[criteria.criteriaKey].forEach(
          (item, index) => {
            if (item.veld === veld) {
              state.huidigeFilter.criteria[criteria.criteriaKey].splice(
                index,
                1
              );
            }
    ...
    const changeLeeftijd = (criteria, leeftijdOpDatum, jongerDan, ouderDan) => {
      ...
      state.huidigeFilter.criteria[
        criteria.criteriaKey
      ].op31december = leeftijdOpDatum;
    ...
    const changeIndividueleSteekkaart = (criteria, aangepast, datum) => {
      ...
      state.huidigeFilter.criteria[
        criteria.criteriaKey
      ].referentie = DateUtil.formatteerDatumVoorApi(datum);
    };
    ...
    const changeGeenRijksregisternummerCriterium = (
      criteria,
      selectedOptions
    ) => {
      state.huidigeFilter.criteria[criteria.criteriaKey] = selectedOptions;
    ...
    const deactivateCriterium = (criterium) => {
      if (
        ...
        criterium.criteriaKey === "geslacht" ||
        criterium.criteriaKey === "leeftijd" ||
        criterium.criteriaKey === "individuelesteekkaart" ||
        criterium.criteriaKey === "oudleden"
      ) {
    ...
    const setNonActieveKolom = () => {
      state.actieveKolommen = ledenlijstService.indexeerEnGroepeerKolommen(
        state.kolommen
      );
      state.nonActieveKolommen = ledenlijstService.indexeerEnGroepeerNonActieveKolommen(
        state.kolommen
      );
    };
    ...
    const checkSortering = (kolom) => {
      ...
      let mapping = {
        "be.vvksm.groepsadmin.model.column.VVKSMGroepsNamenColumn":
          "be.vvksm.groepsadmin.model.column.VVKSMGroepenColumn",
        "be.vvksm.groepsadmin.model.column.VVKSMGroepsNummersColumn":
          "be.vvksm.groepsadmin.model.column.VVKSMGroepenColumn",
    ...
    const filterToepassen = () => {
      ...
      RestService.patchHuidigeFilter(state.huidigeFilter).then((res) => {
        state.huidigeFilter = res.data;
        state.leden = [];
        getLeden(0);
        getFilters();
        state.criteria = ledenlijstFilter.getCriteria();
        state.activeCriteria = ledenlijstFilter.getActieveCriteria(
          res.data,
    ...
    const filteredexportMenuItems = computed(() => {
      return state.menuItems.filter((obj) => {
        return (
          obj.condition &&
          (obj.label.toLowerCase().includes("pdf") ||
            obj.label.toLowerCase().includes("csv"))
        );
      });
    });
    ...
    const veranderFilter = (filter) => {
      ...
      if (state.huidigeFilter.id !== filter.id) {
        RestService.getFilterOpId(filter.id).then((res) => {
          if (res.status === 200) {
            let nieuweFilter = {
            ...
            RestService.patchHuidigeFilter(nieuweFilter).then((res) => {
              state.huidigeFilter = res.data;
              state.offset = 0;
              getLeden(0);
              getKolommen();
    ...
    const activeerAlleGroepen = (criteria) => {
      criteria.items.forEach((item) => {
        if (
          !state.huidigeFilter.criteria[criteria.criteriaKey].includes(
            item.value
          )
        ) {
      ...
      criteria.itemgroups.forEach((group) => {
        group.items.forEach((item) => {
          if (
            ...
            let index = state.huidigeFilter.criteria[
              criteria.criteriaKey
            ].indexOf(item.value);
    ...
    const activeerFunctie = (criteria, functie) => {
      ...
      if (
        ...
        let index = state.huidigeFilter.criteria[criteria.criteriaKey].indexOf(
          functie.value
        );
        state.huidigeFilter.criteria[criteria.criteriaKey].splice(index, 1);
      } else {
        ...
        if (group.label === groepering) {
          group.items.forEach((item) => {
            if (
              ...
              state.huidigeFilter.criteria[criteria.criteriaKey].push(
                item.value
              );
            }
            item.activated = true;
    ...
    const selectLid = (event) => {
      if (
        !event.originalEvent.target.firstElementChild?.className.includes(
    ...
    const activeerKolommen = () => {
      state.actieveKolommen = ledenlijstService.activeerKolommen(
        state.huidigeFilter,
        state.kolommen
      );
      state.nonActieveKolommen = ledenlijstService.indexeerEnGroepeerNonActieveKolommen(
            ...
            .then((res) => {
              if (res.data.length > 0) {
                state.messageDialogMessage =
                  "De lidkaart van volgende leden kan niet worden afgedrukt: </br>" +
                  res.data.join("</br> ") +
                  "</br></br>Deze zijn mogelijk aangesloten bij een andere groep of ploeg.";
