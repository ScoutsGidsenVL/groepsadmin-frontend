...
export default {
  groepeerFilters(filters) {
            ...
            if (!groepByNummer) {
              store.dispatch("getGroepByNummer", groep).then((res) => {
                if (categorisedFilters[res.groepsnummer]) {
                  categorisedFilters[res.groepsnummer]["naam"] =
                    "Gedeeld met " + res.naam + " [" + res.groepsnummer + "]";
                }
              });
            } else {
              if (categorisedFilters[groepByNummer.groepsnummer]) {
                categorisedFilters[groepByNummer.groepsnummer]["naam"] =
                  "Gedeeld met " +
                  groepByNummer.naam +
                  " [" +
                  groepByNummer.groepsnummer +
                  "]";
              }
            }
          });
    ...
    _.forEach(categorisedFilters, (filterGroup) => {
      ...
      let groupedAndSortedFilters = filterGroup.filters.sort(function (a, b) {
        return a.naam.toUpperCase() > b.naam.toUpperCase()
          ? 1
          : b.naam.toUpperCase() > a.naam.toUpperCase()
          ? -1
          : 0;
      });
    ...
    _.forEach(sortedFilters, (filterGroup) => {
      if (filterGroup.isHeader) {
        ...
        let filter = {
          label: filterGroup.naam,
          code: filterGroup.naam,
          items: items,
        };
  ...
  activateCriterium(crit, huidigeFilter) {
    ...
    if (
      !crit.multiValues &&
      !(
        crit.criteriaSubKey === "verbonds" ||
        crit.criteriaSubKey === "groepspecifiek"
      )
    ) {
  ...
  maakFunctieGroepen(functies) {
    ...
    verbondFuncties.forEach((verbondFunctie) => {
      ...
      verbondFunctie.groeperingen.forEach((groepering) => {
        let itemGroupObj = _.find(functieGroep.itemgroups, {
          label: groepering.naam,
        });
  ...
  maakGroepSpecifiekeFunctieGroepen(functies) {
    ...
    arrGroepFunctieGroepen.forEach((v) => {
      ...
      let functieGroep = this.maakFunctieGroep(
        arrMappedObjs,
        "Functies van " + v
      );
      ...
      _.forEach(groepen, (groep) => {
        ...
        if (groep.id === value[0]) {
          ...
          if (value[1].schema.length > 0) {
            groepsObject.items = _.map(
              value[1].schema,
              function (groepseigenGegeven) {
                if (
                  groepseigenGegeven != null &&
                  groepseigenGegeven.type === "lijst"
                ) {
                  return {
                ...
                if (
                  groepseigenGegeven &&
                  groepseigenGegeven.type === "vinkje"
                ) {
                  return {
                    activated: false,
                    veld: groepseigenGegeven.id,
  ...
  getActieveCriteria(huidigeFilter, criteria) {
                    ...
                    if (crit.value && crit.value.length > 0) {
                      crit.value.forEach((value) => {
                        if (item.veld === value.veld) {
                          item.activated = true;
                          item.waarde =
                            value.waarde === "true" ? true : value.waarde;
    ...
    if (
      !Object.prototype.hasOwnProperty.call(huidigeFilter.criteria, "oudleden")
    ) {
      criteria.arrCriteria.forEach((crit) => {
