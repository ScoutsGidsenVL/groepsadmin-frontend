...
export default function apiClient() {
  ...
  if (
    !window.location.pathname.startsWith("/groepsadmin/frontend/formulier/")
  ) {
    ...
    (error) => {
      if (error.response.status === 500) {
        internalInstance.appContext.config.globalProperties.$toast.add({
          ...
          detail:
            "Laat je even aan groepsadministratie@scoutsengidsenvlaanderen.be weten wat je aan het doen was?",
          life: 8000,
        });
      }
