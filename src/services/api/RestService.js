...
export default {
  ...
  zoekGelijkaardig(voornaam, achternaam) {
    return apiClient().get(baseUrl + "zoeken/gelijkaardig", {
      params: { voornaam: voornaam, achternaam: achternaam },
    });
  },
  ...
  verwijderFilter(id) {
    return apiClient().delete(
      baseUrl + "ledenlijst/filter/" + id + "?bevestig=true"
    );
  },
  ...
  verwijderAanvraagMail(id, mail) {
    return apiClient().delete(
      baseUrl + "lid/aanvraag/" + id + "?aanvaard=nee&sendMail=" + mail
    );
  },
  ...
  updateLid(id, lid, bevestig) {
    return apiClient().patch(
      baseUrl + "lid/" + id + "?bevestig=" + bevestig,
      lid
    );
  },
  ...
  getGroepseigenGegevens(groepsnummer) {
    return apiClient().get(
      baseUrl + "groep/groepseigengegevens/" + groepsnummer
    );
  },
  ...
  voegLidToeAanActiviteit(activiteitId, aanwezigheid) {
    return apiClient().post(
      baseUrl + "activiteit/" + activiteitId,
      aanwezigheid
    );
  },
  ...
  voegLedenToeAanActiviteit(activiteitId, leden) {
    return apiClient().post(
      baseUrl + "activiteit/" + activiteitId + "/lijst",
      leden
    );
  },
  ...
  aanwezigheidAanpassen(aanwezigheid) {
    return apiClient().patch(
      baseUrl + "aanwezigheid/" + aanwezigheid.id,
      aanwezigheid
    );
  },
  ...
  getAlleGeregistreerdeLeden(activiteitId) {
    return apiClient().get(
      baseUrl + "aanwezigheid/geregistreerdeleden/" + activiteitId
    );
  },
  ...
  getAlleInAanmerkingKomendeLeden2(activiteitId) {
    return apiClient().get(
      baseUrl + "aanwezigheid/ledenOphalenVoorActiviteit/" + activiteitId
    );
  },

  getFiscaalAttest(groepsnummer, attestParams) {
    return apiClientZip().get(
      baseUrl + "groep/" + groepsnummer + "/fiscaalattest?" + attestParams
    );
  },
};
