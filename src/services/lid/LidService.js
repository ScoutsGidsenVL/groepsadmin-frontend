  ...
  lidSpace() {
    ...
    const state = reactive({
      ...
      messageDialogMessage:
        "De lidkaart is nog niet beschikbaar. <br/>Meer info bij jouw leiding",
      messageDialogHeader: "",
      watchable: false,
      changes: false,
      id: "",
    ...
    const setGeboorteDatum = () => {
      state.lid.vgagegevens.geboortedatum = DateUtil.formatGeboortedatum(
    ...
    const getLid = (id) => {
      ...
      RestService.getLid(id)
        .then(async (res) => {
          ...
          setGeboorteDatum();
          try {
            const attestRes = await RestService.controleerBeschikbaarheidAttest(
              state.lid.id
            );
            heeftFiscaalAttest.value =
              attestRes.status >= 200 &&
              attestRes.status < 300 &&
              attestRes.data;
          } catch (error) {
    ...
    const sorteerFuncties = () => {
      ...
      let inactieveGroepen = Object.entries(ongesorteerdeFuncties).filter(
        ([key]) => !ongesorteerdeFuncties[key].active
      );
      inactieveGroepen.forEach((groep) => {
        store.dispatch("addGroep", groep[0]);
      });

      state.gesorteerdeFuncties = Object.keys(ongesorteerdeFuncties)
        .sort()
        .reduce((obj, key) => {
    ...
    const filterGroepsEigenVelden = () => {
      state.groepseigenVelden = Object.fromEntries(
        Object.entries(state.lid.groepseigenVelden).filter(
          ([key]) => state.lid.groepseigenVelden[key].schema.length > 0
        )
      );
    };
    ...
    const stopAlleFuncties = () => {
      ...
      confirm.require({
        message:
          ...
          ", je staat op punt om al je functies bij Scouts en Gidsen Vlaanderen te schrappen. " +
          " <br/>" +
          "Ben je zeker? <br/><br/>" +
    ...
    const opslaan = () => {
      ...
      if (v.value.$invalid) {
        ...
        toast.add({
          ...
          summary: "Wijzigingen",
          detail:
            "Kan nog niet opslaan. Er zijn nog fouten vastgesteld in het formulier. </br> Kijk zeker ook adressen en contacten na.",
          life: 3000,
        });
        return;
      }
      ...
      if (
        state.gewijzigdLid.functies &&
        state.gewijzigdLid.functies.length > 0
      ) {
        state.gewijzigdLid.functies.forEach((functie) => {
          if (
            ...
            confirm.require({
              message:
                "Je maakt iemand anders dan jezelf VGA binnen de huidige groep, je zal zelf deze functie verliezen! Je wordt uitgelogd indien je bevestigt. Ben je zeker?",
              header: "Nieuwe VGA",
              icon: "pi pi-exclamation-triangle",
              accept: () => {
                confirmVGA = true;
    ...
    const voegFunctieToe = (obj) => {
      ...
      if (state.lid && state.lid.functies) {
        for (let [index, val] of state.lid.functies.entries()) {
          if (
            val.functie === obj.functie.functie &&
            val.groep === obj.groepsnummer &&
            !val.einde
          ) {
    ...
    const saveLid = (confirmVGA, vga) => {
      ...
      if (state.gewijzigdLid.groepseigenVelden && state.eigenProfiel) {
        _.forOwn(
          state.gewijzigdLid.groepseigenVelden,
          function (groepseigenVelden) {
    ...
    const updateFuncties = (functie) => {
      ...
      RestService.updateLid(state.lid.id, lid, bevestig)
        .then((res) => {
          if (res.status === 200) {
            toast.add({
              severity: "success",
            ...
            state.lid = res.data;
            state.lid.vgagegevens.geboortedatum = DateUtil.formatteerDatum(
              state.lid.vgagegevens.geboortedatum
            );
            sorteerFuncties();
            filterGroepsEigenVelden();
    ...
    const isEigenProfiel = computed({
      get() {
        return (
          route.params.id === "profiel" ||
          store.getters.profiel.id === route.params.id
        );
      },
    });
          ...
          if (res.data.length > 0) {
            if (state.lid.functies.length > 0) {
              state.messageDialogMessage =
                "De lidkaart is pas beschikbaar wanneer de groep of ploeg </br>van jouw belangrijkste functie de leden heeft verbeterd. </br> Voor sommige functies zijn geen lidkaarten beschikbaar";
            }
