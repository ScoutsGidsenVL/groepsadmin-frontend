...
export default {
  lidToevoegenSpace() {
    ...
    onMounted(() => {
      ...
      RestService.getLid("profiel").then((res) => {
        ...
        if (res.data.groepseigenVelden[state.groepsnummer].schema.length > 0)
          state.groepseigenVelden = {
            [state.groepsnummer]:
              res.data.groepseigenVelden[state.groepsnummer],
          };
        // Voeg de waardes die werden opgegeven op het inschrijvingsformulier in
        state.groepseigenVelden[state.groepsnummer].waarden = {};
        state.lid.groepseigenVeldenAanvraag.forEach(
          (groepseigenVeldAanvraag) => {
            state.groepseigenVelden[state.groepsnummer].waarden[
              groepseigenVeldAanvraag.id
            ] = groepseigenVeldAanvraag.waarde;
          }
        );
      });
    ...
    const opslaan = () => {
      ...
      if (
        !state.lid.functies ||
        (state.lid.functies && state.lid.functies.length === 0)
      ) {
        toast.add({
          severity: "warn",
          summary: "Wijzigingen",
      ...
      if (state.lid.vgagegevens.geboortedatum) {
        ...
        state.lid.vgagegevens.geboortedatum = DateUtil.formatteerDatumVoorApi(
          geboortedatum
        );
      }
      ...
      if (state.groepseigenVelden[state.groepsnummer]) {
        state.lid.groepseigenVelden = {
          [state.groepsnummer]: state.groepseigenVelden[state.groepsnummer],
        };
      }
      ...
      RestService.saveNieuwLid(state.lid)
        .then((res) => {
          if (res.status === 201) {
            toast.add({
              severity: "success",
            ...
            state.changes = false;
            if (state.lid.id) {
              RestService.verwijderAanvraag(state.lid.id)
                .then((result) => {
