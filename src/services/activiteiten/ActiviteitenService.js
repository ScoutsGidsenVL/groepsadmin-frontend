...
export default {
  activiteitenSpace() {
    ...
    const voegLedenToe = (leden) => {
      ...
      if (state.geselecteerdeActiviteiten.length > 0) {
        state.geselecteerdeActiviteiten.forEach((activiteit) => {
          RestService.voegLedenToeAanActiviteit(activiteit.id, ledenIds)
            .then(() => {
              toast.add({
                severity: "success",
    ...
    const registreerAanwezigheden = (activiteitId) => {
      router.push({
        name: "Aanwezigheden",
        params: { activiteit: activiteitId },
      });
    };
    ...
    const bevestigVerwijderen = () => {
      ...
      RestService.verwijderActiviteit(state.teVerwijderenActiviteitId)
        .then(() => {
          state.activiteiten.forEach((activiteit) => {
            if (activiteit.id === state.teVerwijderenActiviteitId) {
              state.activiteiten.splice(
                state.activiteiten.indexOf(activiteit),
                1
              );
            }
          });
    ...
    const verwijderActiviteit = (activiteitId) => {
      ...
      state.dialogMessage =
        "Je staat op het punt om deze activiteit te verwijderen. </br> Dit kan niet ongedaan worden gemaakt. Ben je zeker?";
      ...
      store.getters.groepen.forEach((groep) => {
        if (rechtenService.hasPermission("functies." + groep.groepsnummer)) {
          store.getters.functies.forEach((functie) => {
            if (
              functie.groeperingen[0].naam !== "Leiding" &&
              functie.groeperingen[0].naam !== "Anderen"
            ) {
    ...
    const getActiviteiten = () => {
      state.isLoadingActiviteiten = true;
      RestService.getActiviteiten(state.selectedGroep.groepsnummer)
        .then((res) => {
          if (res.status === 200) {
            state.activiteiten = res.data.activiteiten;
            state.activiteiten.forEach((activiteit) => {
              activiteit.werkjaar =
                activiteit.werkjaar + "-" + (activiteit.werkjaar + 1);
              activiteit.prijs = Number.parseFloat(activiteit.prijs).toFixed(2);
              const aantalDagen = Math.round(
                (new Date(activiteit.tot).getTime() -
                  new Date(activiteit.van).getTime()) /
                  (1000 * 3600 * 24) +
                  1
              );
    ...
    const formatteerPeriode = (activiteit) => {
      return (
        DateUtil.formatteerDatum(activiteit.van) +
        " - " +
        DateUtil.formatteerDatum(activiteit.tot)
      );
    };
  ...
  activiteitenDialogSpace(props) {
    ...
    const opslaan = () => {
      ...
      state.activiteit.van = DateUtil.formatteerDatumVoorApi(
        state.activiteit.van
      );
      state.activiteit.tot = DateUtil.formatteerDatumVoorApi(
        state.activiteit.tot
      );
    ...
    const activiteitOpslaan = () => {
      ...
      RestService.activiteitOpslaan(state.activiteit)
        .then((res) => {
          if (res.status === 200) {
            toast.add({
              severity: "success",
    ...
    const activiteitAanpassen = () => {
      ...
      state.activiteit.van = new Date(state.activiteit.van);
      state.activiteit.tot = new Date(state.activiteit.tot);
      RestService.activiteitAanpassen(state.activiteit)
        .then((res) => {
          if (res.status === 200) {
            toast.add({
              severity: "success",
      ...
      attest: {
        kbo: {
          required: helpers.withMessage(
            "Pseudo-KBO-nummer is verplicht",
            required
          ),
        },
        certnaam: {
          required: helpers.withMessage(
            "Naam certificerende instantie is verplicht",
            required
          ),
        },
        certadres: {
          required: helpers.withMessage(
            "Adres certificerende instantie is verplicht",
            required
          ),
        },
        certpostcode: {
          required: helpers.withMessage(
            "Postcode certificerende instantie is verplicht",
            required
          ),
        },
        certgemeente: {
          required: helpers.withMessage(
            "Gemeente certificerende instantie is verplicht",
            required
          ),
        },
        certkbo: {},
    ...
    const genereerAttest = () => {
      ...
      if (v.value.$invalid) {
        ...
        toast.add({
          ...
          summary: "Wijzigingen",
          detail:
            "Kan nog niet indienen. Er zijn nog fouten vastgesteld in het formulier.",
          life: 3000,
        });
        return;
      } else {
        const attestParams = new URLSearchParams(state.attest).toString();
        RestService.getFiscaalAttest(props.groep.groepsnummer, attestParams)
          .then((res) => {
            if (res.data) {
