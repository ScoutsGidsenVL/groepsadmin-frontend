...
export default {
  sortedKeys(obj) {
    return _.sortBy(Object.keys(obj), [
      function (key) {
        if (/^[0-9]+$/.test(key)) {
          return Number(key);
        } else {
  ...
  getLedenaantalPerLeeftijd(ledenaantallenData) {
    ...
    _.forEach(
      this.sortedKeys(ledenaantallenData.ledenPerLeeftijd),
      function (keySoort, index) {
        let values = _.fill(new Array(alleJarenUniek.length), 0);
        _.forEach(
          ledenaantallenData.ledenPerLeeftijd[keySoort],
          function (valueAantal, keyJaar) {
  ...
  getGroepsevolutie(ledenaantallenData) {
    ...
    let data = {
      labels: this.sortedKeys(
        ledenaantallenData.groepsevolutie[0].aantalPersonen
      ),
      datasets: [],
    };
  ...
  tekenHuidigeLeidingsErvaring(ledenaantallenData) {
    let chartHoverColors = [
      "rgba(232, 232, 96, 0.62)",
      "rgba(141, 221, 119, 0.62)",
      "rgba(236, 148, 76, 0.59)",
      "rgba(76, 83, 236, 0.59)",
      "rgba(212, 94, 94, 0.59)",
      "rgba(120, 97, 218, 0.59)",
    ];
    let chartColors = [
      "rgba(232, 232, 96, 1)",
      "rgba(141, 221, 119, 1)",
    ...
    let data = {
      labels: this.sortedKeys(ledenaantallenData.leidingservaring).map(
        function (jaar) {
          return jaar + " jaar";
        }
      ),
  ...
  tekenInEnUitstroom(ledenaantallenData) {
    ...
    let data = {
      labels: this.sortedKeys(
        ledenaantallenData.uitstroom[0].aantalPerLeeftijd
      ),
      datasets: [],
    };
    ledenaantallenData.uitstroom.forEach((value, index) => {
      data.datasets.push({
        ...
        borderColor:
          chartColors[(index + 2 * Math.round(index / 6)) % 6].border,
