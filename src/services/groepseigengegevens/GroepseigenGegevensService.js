...
export default {
  ...
  groepSpace(props) {
    ...
    const wisKeuze = (index, element) => {
      confirm.require({
        message:
          "Ben je zeker dat je " + element.keuzes[index] + " wil verwijderen?",
    ...
    const voegGeigToe = () => {
      if (!state.groep.groepseigenGegevens)
        state.groep.groepseigenGegevens = [];
      let newGegeven = {
        beschrijving: null,
    ...
    const geigTitel = (gegeven) => {
      if (
        gegeven.status === "nieuw" &&
        (gegeven.label === "Nieuw groepseigen gegeven" || gegeven.label === "")
      ) {
